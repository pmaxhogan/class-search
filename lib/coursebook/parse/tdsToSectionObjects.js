...
import {isDays, isNormalTimes, isSpecialDays, splitIntoDays} from "./daysTimesParse.js";
...
export function tdsToSectionObjects(tds, prefix) {
    ...
    if (noLocationHeaders.includes(scheduleAndLocationParts[0]) || !scheduleAndLocationParts.length) {
        ...
        const foundDays = dedupe(scheduleAndLocationParts.filter((x) => isDays(x)));
        const foundTimes = dedupe(scheduleAndLocationParts.filter((x) => isNormalTimes(x)));
