...
export async function findSectionsInLocation({building, room, floor}) {
    return (await getAllSections()).filter(courseSection => courseSection.section.location.room === room && courseSection.section.location.floor === floor && courseSection.section.location.building === building);
}
