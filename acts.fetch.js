...
{
    const sessionId = await getFromCache("utd-sessionId");
...
const getCourseData = async (courseCode, afterConfirmation = false) => {
    ...
    if(!requestXml) {
        console.log("cache not defined ", afterConfirmation);
        const icyPos = 0;
...
// console.log(await getCourseData("ACTS"));
console.log(await getCourseData("ACCT"));
